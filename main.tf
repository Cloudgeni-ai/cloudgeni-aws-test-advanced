...
resource "aws_cloudtrail" "insecure_trail" {
  ...
  kms_key_id                    = aws_kms_key.secure_key.arn # Encryption enabled (best practice)
}
...
resource "aws_kms_key" "secure_key" {
  ...
  enable_key_rotation     = true # Key rotation enabled (best practice)
  
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
    {
      Sid       = "Enable Secure IAM Permissions"
      Effect    = "Allow"
      Principal = {
        AWS = "arn:aws:iam::066730217701:role/SecureCloudTrailRole"
      }
      Action    = "kms:Decrypt"
      Resource  = "*"
    }
